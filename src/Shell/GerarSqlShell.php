...
class GerarSqlShell extends Shell {
    ...
    public function main() {
        ...
        $this->out('Creating table structure.');
        $this->install();
        $this->out('Creating basic user data.');
        $this->dados();
        $this->out('Finishing the installation.');
    }
    ...
    public function install() {
        ...
        if (count($tables) > 0) {
            ...
            foreach ($tables as $key => $value) {
                ...
                $sql[] = '-- Starting the table structure `' . $value . '` --';
                ...
                $sql[] = '-- End of table structure `' . $value . '` --';
                $sql[] = "\n";
            }
        }
    ...
    public function dados() {
        ...
        /* if (count($tables) > 0) {
          foreach ($tables as $key => $value) {
          $sql[$value] = implode("\n ", $this->findDados($value, [], 'REPLACE'));
          }
          } */

        $sql['contacts'] = implode("\n ", $this->findDados('contacts', [], 'INSERT IGNORE'));
        $sql['phones'] = implode("\n ", $this->findDados('phones', [], 'INSERT IGNORE'));
