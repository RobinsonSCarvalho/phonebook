...
class MyFormHelper extends BootstrapFormHelper {
    ...
    public function habilita_noticias($fieldName, array $options = []) {
        $default = [
            'empty' => 'Selecione',
            'options' => [
                0 => 'Habilitado',
    ...
    public function tipo_contact($fieldName, array $options = []) {
        $default = [
            'empty' => 'Selecione',
            'options' => [
                'phone' => 'Phone',
    ...
    public function tipo_banner($fieldName, array $options = []) {
        $default = [
            'empty' => 'Selecione',
            'options' => [
                0 => 'Grande Splash (720px x 1230px)',
                1 => 'Pequeno (320px x 50px)'
            ]
        ];
        $options = \Cake\Utility\Hash::merge($default, $options);
        return $this->input($fieldName, $options);
    ...
    public function habilita_programacao($fieldName, array $options = []) {
        $default = [
            'empty' => 'Selecione',
            'options' => [
                0 => 'Habilitado',
    ...
    public function habilita_propaganda($fieldName, array $options = []) {
        $default = [
            'empty' => 'Selecione',
            'options' => [
                0 => 'Habilitado',
    ...
    public function bootstrapTags($fieldName, array $options = []) {
        $default = [
            ...
            'type' => 'text',
            'help' => 'Separação da palavra chave por virgula ou tab',
            'script' => [
                'delimiter' => ';'
            ]
        ];
    ...
    public function fileUpload($fieldName, array $options = array()) {
        ...
        if (isset($options['path']) AND ! empty($options['path'])) {
            ...
            //debug($list);
            $image = '<span class="text-center"><img class="img-lg img-border img-thumbnail" src="data:image/' . $list['extension'] . ';base64,' . base64_encode(file_get_contents($list['dirname'] . '/' . $list['basename'])) . '"></span>';
            unset($options['path']);
        }
        $id = $this->_domId($fieldName);
        ...
        return '<div class="form-group file col-xs-12 col-md-4" ><div class="listFileUpload">' . $image . '</div>' . $this->input($fieldName, $options) . '</div>';
    }
    ...
    public function social_options($sociais_tipos, $valor) {
        $options = [];
        foreach ($sociais_tipos as $key => $value) {
            if ($key === $valor) {
    ...
    public function social_li($dados = [], $sociais_tipos = []) {
        $retorno = array();
        if (count($dados) > 0) {
            foreach ($dados as $key => $value) {
                $retorno[] = "<li class='list-group-item' id='div_" . $key . "'><div class='row'><div class='col-xs-12' style='padding-left: 25px;'><div class='input-group'><span class='input-group-addon' id='basic-addon" . $key . "' style='padding: 0px 5px;'><select class='mudanca_tipo' id='" . $key . "' style='margin: 0px; background: none; border: 0px;'>" . $this->social_options($sociais_tipos, $value['tipo']) . "</select></span><input type='hidden' name='social[" . $key . "][tipo]' value='" . $value['tipo'] . "' id='social-" . $key . "-tipo'><input type='text' name='social[" . $key . "][valor]' class='form-control' aria-describedby='basic-addon" . $key . "' id='social-" . $key . "-valor' value='" . $value['valor'] . "'></div></div></div></li>";
            }
        }
    ...
    public function phone_li($dados = []) {
        $retorno = array();
        if (count($dados) > 0) {
            foreach ($dados as $key => $value) {
                $retorno[] = "<li class='list-group-item' id='div_" . $key . "'><input type='text' name='phone[" . $key . "][valor]' class='form-control phone' aria-describedby='basic-addon" . $key . "' id='phone-" . $key . "-valor' value='" . $value['phone'] . "'></li>";
            }
        }
        return implode(' ', $retorno);
    }
