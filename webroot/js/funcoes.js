    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.retorno.status == 'OK') {
                ...
                if (!!cidade_id) {
                    cake.util.cidades($('#estado-id'), '#cidade-id', function () {
                        $('#cidade-id').val(result.retorno.Cep[cidade_id]);
                    });
                }
...
cake.util.cidades = function (obj, campo, callback) {
    $.ajax({
...
cake.util.mascara = function () {
    ...
    $('input.phone').mask('(99) 99999-999?9');
    ...
    $('input.numeroNegative').each(function () {
        $(this).maskMoney('destroy');
        $(this).maskMoney({prefix: $(this).attr('data-prefix'), allowNegative: $(this).attr('data-allowNegative'), thousands: $(this).attr('data-thousands'), decimal: $(this).attr('data-decimal'), affixesStay: $(this).attr('data-affixesStay')});
    });
...
$('.botao_add_phone').click(function () {
    var selectbox = '';
    var retorno = 'phone';
    if ($(this).attr('id') !== 'botao_phone') {
        retorno = 'phone_contact';
    }
    var identificacao = $.now();
    var conteudo = "<li class='list-group-item' id='div_" + identificacao + "'><input type='text' name='phone[" + identificacao + "][valor]' class='form-control phone' aria-describedby='basic-addon" + identificacao + "' id='phone-" + identificacao + "-valor'></li>";
    $('#phones_contacts').prepend(conteudo);
    cake.util.mascara();
});
